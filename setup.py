...
def build_c_re2():
    ...
    if not os.path.exists(C_RE2_PREFIX_DIR):
        os.makedirs(C_RE2_PREFIX_DIR)
    if not os.path.exists(os.path.join(C_RE2_SOURCE_DIR, 'Makefile')):
        # Obtained via:
        #   $ curl -L https://github.com/docker-hub/re2/archive/master.tar.gz -o c_re2.tar.gz
        subprocess.check_call(['tar', 'zxf', os.path.join(ROOT_DIR, 'c_re2.tar.gz'), '-C', ROOT_DIR])

    env = os.environ.copy()
    env['PREFIX'] = C_RE2_PREFIX_DIR
    subprocess.check_call(['make', '-C', C_RE2_SOURCE_DIR, 'install'], env=env)
